...
with st.sidebar:
    ...
    # Pro section
    if not is_pro:
        st.info(f"🆓 免费 · 剩余 {remaining}/{FREE_SEARCH_LIMIT} 次搜索")
        if not is_guest:
            st.button("💎 升级 Pro",use_container_width=True,key="pro_sidebar_btn",
                      help=f"${PRO_PRICE}/永久",
                      on_click=lambda: st.session_state.update({"show_pro_inline":True,"nav_tab":"🔍 搜索"}))
    else:
        st.success("💎 Pro 会员 · 无限搜索")

    if st.session_state.get("show_pro_inline"):
        with st.expander("💎 升级 Pro 会员",expanded=True):
            ...
            **价格: ${PRO_PRICE}/永久**
            ...
            if st.button("💳 确认升级",type="primary",use_container_width=True):
                u=upgrade_to_pro(user["id"])
                st.session_state.user=u;st.session_state.show_pro_inline=False
                st.success("升级成功！💎");st.rerun()
    ...
    if is_guest:
        ...
        role_text=f"🎓 {user.get('role','未选择')}"
        pro_label="💎 Pro" if is_pro else "🆓 免费"
        st.markdown(f"#### 👤 {nn}")
        st.caption(f"{role_text} · {pro_label}")
        st.caption(f"📧 {user.get('email','')}")
        if st.button("✏️ 编辑资料",use_container_width=True): st.session_state.show_edit=not st.session_state.get("show_edit",False)
...
if st.session_state.get("show_edit") and not is_guest:
    with st.sidebar.expander("✏️ 编辑资料",expanded=True):
...
if not is_guest and not user.get("role"):
    st.warning("💡 请设置学术身份，点击左侧「✏️ 编辑资料」设置。")


# ============================================================
# === Tab: 搜索 ===
# ============================================================
if "搜索" in current_tab and "订阅" not in current_tab and "历史" not in current_tab:
    ...
    with left:
        ...
        st.caption({"keyword":"⚡ 快速","agent":"🧠 Agent"}[st.session_state.search_mode])

        keyword=st.text_input("关键词",placeholder="例如：Transformer、GNN…",key="search_keyword",label_visibility="collapsed")
        ri=""
        if st.session_state.search_mode=="agent":
            ri=st.text_area("研究兴趣",placeholder="用自然语言描述研究兴趣…",key="ri_input",label_visibility="collapsed")

        search_count=st.slider("数量",1,10,5,1)
        to=st.selectbox("时间",["不限时间","近半年","近 1 年","近 3 年","自定义"],0)
        ...
        if to=="近半年": yf_val=yr if datetime.now().month>6 else yr-1
            ...
            c1,c2=st.columns(2)
            with c1: a=st.number_input("起",1900,yr,yr-5,1)
            with c2: b=st.number_input("止",1900,yr,yr,1)
        ...
        if st.button("🔍 搜索",type="primary",use_container_width=True):
            if not is_guest and not is_pro:
                ...
                if not allowed: st.warning("免费搜索次数已用完，点击左侧「💎 升级 Pro」");st.stop()
            ...
            if st.session_state.search_mode=="agent":
                ...
                if not interest: st.warning("请输入研究兴趣或关键词")
                    ...
                    st.session_state.search_count=search_count;st.session_state.search_offset=0
                    st.session_state.seen_paper_keys=[];st.session_state.confirm_new_search=False
                    with st.spinner("AI Agent 正在规划…"):
                        ...
                        state["max_total_results"]=search_count;state=retrieval_agent(state)
                    results=state.get("search_results",[])
                    st.session_state.seen_paper_keys=[paper_key(r) for r in results]
                    batch_enrich(results);st.session_state.search_results=results
                    if not is_guest: save_search_history(user["id"],keyword.strip() or interest[:40],results)
                    st.rerun()
            elif not keyword.strip(): st.warning("请输入关键词")
            else:
        ...
        if st.session_state.search_results:
            ...
            with cn:
                if st.button("🔄 重新搜索",use_container_width=True):
                    ...
                    else: st.session_state.search_results=[];st.session_state.seen_paper_keys=[];st.rerun()

        st.divider()
        st.subheader(f"📋 已选（{len(st.session_state.selected_papers)} 篇）")
    ...
    with right:
        if st.session_state.confirm_new_search:
            ...
            if not papers: st.warning("没有已选论文");st.session_state.generating=False
                ...
                status.info("📄 生成 HTML 报告…");prog.progress(75)
            ...
            with cd:
                with open(st.session_state.zip_path,"rb") as f:
                    lb="下载全部（论文+报告）" if st.session_state.report_md else "下载 zip"
            ...
            if len(results)==0: st.warning("未找到匹配论文")
            ...
            st.markdown("""<div style="text-align:center;padding:60px 20px">
              ...
              <h2>开始你的学术探索之旅</h2>
              <p style="color:#888;font-size:16px">在左侧输入研究方向，发现领域最前沿的论文</p>
...
if "订阅" in current_tab:
    ...
    elif not user.get("email"): st.warning("请先在编辑资料中设置邮箱。")
    else:
