...
def init_db():
    ...
    try:
        db.execute("ALTER TABLE users ADD COLUMN is_pro INTEGER DEFAULT 0")
    except Exception:
        pass
    try:
...
def check_search_limit(user_id: int) -> tuple:
    """Returns (allowed: bool, remaining: int, is_pro: bool, message: str)"""
    db = _conn()
    try:
        ...
        is_pro = u["is_pro"] == 1
        if is_pro:
            return True, 999, True, "Pro ??????????"
        used = u["search_count"] or 0
        remaining = FREE_SEARCH_LIMIT - used
        if remaining <= 0:
            return False, 0, False, f"??????????{FREE_SEARCH_LIMIT} ?????? Pro ????"
        return True, remaining, False, f"?? {remaining} ?????"
    except Exception as e:
        return False, 0, False, f"????: {e}"
    finally:
        db.close()
...
def increment_search_count(user_id: int):
    db = _conn()
    try:
        db.execute("UPDATE users SET search_count = search_count + 1 WHERE id=?", (user_id,))
        db.commit()
    finally:
        db.close()


def upgrade_to_pro(user_id: int) -> tuple:
    """Upgrade a user to Pro. Returns (success, message)."""
    db = _conn()
    try:
        u = db.execute("SELECT * FROM users WHERE id=?", (user_id,)).fetchone()
        if not u:
...
def subscribe(user_id: int, keyword: str) -> tuple:
    ...
    try:
        ...
        if existing:
            return False, f"????\u300c{keyword}\u300d"
        db.execute(
            "INSERT INTO subscriptions (user_id, keyword) VALUES (?,?)",
            (user_id, keyword.strip()),
        )
        db.commit()
        return True, f"???\u300c{keyword}\u300d????????? {u['email']}"
    finally:
        db.close()
